...
export function AppShell({ children }: AppShellProps) {
  ...
  const navItems = [
    { href: "/app", label: terms.app.navigation.dashboard, icon: Activity },
    { href: "/app/cases", label: terms.app.navigation.allMatters, icon: FolderOpen },
    { href: "/app/new-case", label: terms.app.navigation.newMatter, icon: Plus },
  ];
      ...
      <aside className="sticky top-0 hidden h-screen w-72 border-r border-sidebar-border bg-sidebar/95 backdrop-blur-xl lg:flex lg:flex-col">
        <div className="flex h-18 items-center justify-between border-b border-sidebar-border px-5">
          <Link href="/" className="inline-flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-semibold">{terms.marketing.productName}</p>
        ...
        <div className="flex-1 space-y-6 px-4 py-5">
          <div className="relative">
            ...
            <Input
              placeholder="Find matter..."
              className="h-9 rounded-xl bg-background/70 pl-8 text-xs"
            />
          </div>
          ...
          <div className="mb-3 flex items-center gap-3 rounded-xl border bg-background/70 px-3 py-2">
            ...
            <div className="min-w-0">
              <p className="truncate text-sm">
                {session?.user?.name || `${terms.marketing.productName} User`}
              </p>
